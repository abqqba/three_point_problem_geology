...
def get_point(name):
    x = float(input(f"\033[96mMasukkan koordinat X untuk {name}:\033[0m "))
    y = float(input(f"\033[96mMasukkan koordinat Y untuk {name}:\033[0m "))
    z = float(input(f"\033[96mMasukkan koordinat Z untuk {name}:\033[0m "))
    return np.array([x, y, z])

print("Masukkan koordinat tiga titik:")
A = get_point("A")
B = get_point("B")
C = get_point("C")
